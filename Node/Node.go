...
func (i *Info) AcquireObject() {
	...
	if !i.ObjectPresent {
			...
			for _, n := range i.Nodes {
				if n == i.Port { continue }
				msg := Models.Message{
					Source: i.Port,
					Intent: constants.IntentRequestObject,
				}
				if err := i.sendMsg(msg, n); err != nil {
					fmt.Printf("Couldn't send object request: %v\n", err)
				}
			}
	}
}
...
func (i *Info) ReleaseObject() {
	i.Interested = false
	i.Object.Obtained[i.Port]++

	for _, k := range i.Nodes {
		if i.RequestBy[k] > i.Object.Obtained[k] {
			...
			if err := i.sendMsg(msg, k); err != nil {
...
func (i *Info) request(reqSource string) {
	i.RequestBy[reqSource]++
	if i.ObjectPresent && !i.Interested {
		i.ObjectPresent = false
